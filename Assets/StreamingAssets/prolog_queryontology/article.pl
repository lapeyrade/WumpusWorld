...
/* Items */
valuableitem(X) :- object(X), hasValue(X, high).
hasValue(X, high) :- valuableitem(X).
object(X) :- valuableitem(X).
valuableitem(X) :- gold(X).

unvaluableitem(X) :- object(X), hasValue(X, low).
hasValue(X, low) :- unvaluableitem(X).
object(X) :- unvaluableitem(X).
...
hasCharacter(X, f2(X)) :- wumpus(X).
aggressive(f2(X)) :- wumpus(X).
dangerous(X) :- aggressive(X).

property(X) :- hasAspect(X, Y).
property(X) :- hasCharacter(X, Y).
property(X) :- hasValue(X, Y).
