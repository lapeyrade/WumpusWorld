...
% monstre(X) :- wumpus(X).                      % modifié
objetDeValeur(X) :- or(X).
dangereux(X) :- agressif(X).					% ajouté
monstrueux(X) :- oeilUnique(X).                 % ajouté
...
aAspect(X,Y) :- aTraitPhysique(X,Y).		    % ajouté	

% traduction de "monstre \equiv \exists aAspect.monstrueux"
% i.e. "aASpect(X,Y), monstrueux(Y) <-> monstre(X)"
% par skolémisation
monstre(X) :- aAspect(X,Y),monstrueux(Y).       % ajouté
aAspect(X,f3(X)) :- monstre(X).                 % ajouté
monstrueux(f3(X)) :- monstre(X).                % ajouté

% traduction de "wumpus \sqsubseteq \exists aTraitPhysique.oeilUnique"
% i.e. "aTraitPhysique(X,Y), oeilUnique(Y) <- wumpus(X)"
% par skolémisation
aTraitPhysique(X,f4(X)) :- wumpus(X).           % ajouté
oeilUnique(f4(X)) :- wumpus(X).                 % ajouté
